...
class ExportController extends Controller {
    ...
    public function download()
    {
        ...
        if (Input::get('download') == 'excel') {
            ...
            // Check no. of days is less than 20
            $noOfDays = $startDate->diffInDays($endDate);
            if ($noOfDays > 20) {
                return back()->withInput()->withErrors(['startDate' => 'Maximum period is 20 days']);
            }
            return self::downloadExcel($startDate, $endDate, $noOfDays);
        } else if (Input::get('download') == 'csv_humidity') {
            return self::downloadHumidityCSV();
        } else if (Input::get('download') == 'csv_temperature') {
            return self::downloadTemperatureCSV();
        } else {
            return self::downloadPowerCSV();
        }
    }
    ...
    public function downloadExcel($startDate, $endDate, $noOfDays)
    {
        $room = Room::findOrFail(Input::get('room', 1));
        $humiditySensors = array_merge(explode(',', $room->humidity_sensor_names), explode(',', $room->external_humidity_sensor_names), ['humid15']);
        $temperatureSensors = array_merge(explode(',', $room->temperature_sensor_names), explode(',', $room->external_temperature_sensor_names), ['tem15']);
